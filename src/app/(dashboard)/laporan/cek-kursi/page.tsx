...
function rowStyle(status: CekKursiRow["status"]) {
  if (status === "kurang") return "bg-emerald-50";
  if (status === "lebih") return "bg-red-50";
  return "";
}
...
export default function CekKursiPage() {
  ...
  return (
    <div className="p-6 md:p-8 print:p-0">
      ...
      <h1 className="text-2xl font-semibold text-slate-900 mb-1">Cek Kursi</h1>
            ...
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((r, idx) => (
                  <tr
                    key={r.kelas}
                    className={`border-b border-slate-100 last:border-0 ${rowStyle(r.status)}`}
                  >
                    <td className="px-4 py-2.5 text-slate-500">{idx + 1}</td>
                    <td className="px-4 py-2.5 font-medium text-slate-800">{r.kelas}</td>
                    <td className="px-4 py-2.5 text-center text-slate-700">{r.jumlahSiswa}</td>
                    <td className="px-4 py-2.5 text-slate-700">{r.keterangan}</td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      <div className="flex items-center gap-4 mt-4 text-xs text-slate-500 print:hidden">
        <span className="inline-flex items-center gap-1.5">
          <span className="h-3 w-3 rounded bg-emerald-50 border border-emerald-200 inline-block" />
          Masih ada slot kursi
        </span>
        <span className="inline-flex items-center gap-1.5">
          <span className="h-3 w-3 rounded bg-red-50 border border-red-200 inline-block" />
          Melebihi kapasitas
        </span>
      </div>
    </div>
