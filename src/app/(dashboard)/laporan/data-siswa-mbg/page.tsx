...
export default function DataSiswaMbgPage() {
  ...
  return (
    <div className="p-6 md:p-8 print:p-0">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4 print:hidden">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-white border border-slate-300 text-slate-700 text-sm font-medium px-3 py-2 hover:bg-slate-50 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-lg bg-white border border-slate-300 text-slate-700 text-sm font-medium px-3 py-2 hover:bg-slate-50 transition-colors"
          >
            <Download className="h-4 w-4" />
      ...
      <h1 className="text-2xl font-semibold text-slate-900 mb-1">Data Siswa MBG</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          {groups.map((group) => (
            ...
            >
              <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-6 break-inside-avoid">
                <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
                  <p className="text-sm font-semibold text-slate-800">Kelas {group.kelas}</p>
                  <p className="text-xs text-slate-500">{group.siswa.length} siswa</p>
                </div>
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {group.siswa.map((s, idx) => (
                      <tr key={`${group.kelas}-${idx}`} className="border-b border-slate-100 last:border-0">
                        <td className="px-4 py-2 text-slate-500">{idx + 1}</td>
                        <td className="px-4 py-2 font-medium text-slate-800">{s.nama || "-"}</td>
                        <td className="px-4 py-2 text-center text-slate-600">{s.jk || "-"}</td>
                        <td className="px-4 py-2 text-slate-600">{group.kelas}</td>
                        <td className="px-4 py-2 text-slate-600">{s.nisn || "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
