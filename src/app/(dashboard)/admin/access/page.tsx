  ...
  return (
    <div className="p-6 md:p-8 dark:bg-slate-900 min-h-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Hak Akses</h1>
            ...
            <tbody>
              {loading ? (
                ...
                pagedGroups.map((group, idx) => (
                  <tr key={group.userId} className="border-b border-slate-100 dark:border-slate-700/60 last:border-0 hover:bg-slate-50/60 dark:hover:bg-slate-700 align-top">
                    <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{page * PAGE_SIZE + idx + 1}</td>
                    <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-200">{group.nama || "-"}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{group.email || "-"}</td>
                      ...
                      <div className="space-y-2">
                        {group.entries.map((entry) => (
                          <div key={entry.id} className="h-6 flex items-center gap-1.5">
                            {entry.can_view && (
                              <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 text-xs font-medium text-blue-600 dark:text-blue-400">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-slate-700">
            ...
            <div className="flex gap-1">
              <button
                ...
                disabled={page === 0}
                className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 disabled:opacity-40 hover:bg-slate-50 dark:hover:bg-slate-700"
              >
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-md w-full shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1.5">User</label>
                <select
                  value={formUserId}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-1.5">Modul</label>
                <select
                  value={formModule}
              ...
              <div>
                <p className="text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Aksi yang diizinkan</p>
                <div className="space-y-2">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 max-w-sm w-full shadow-xl">
            <h3 className="font-semibold text-slate-900 dark:text-slate-100 mb-1.5">Hapus hak akses ini?</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-5">
              Akses ke modul{" "}
              <span className="font-medium text-slate-700 dark:text-slate-200">
                {MODULE_LABEL[deleteTarget.module] ?? deleteTarget.module}
              </span>{" "}
              untuk <span className="font-medium text-slate-700 dark:text-slate-200">{deleteTarget.nama || deleteTarget.email}</span>{" "}
              akan dihapus.
            </p>
