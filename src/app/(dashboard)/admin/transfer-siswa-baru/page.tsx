...
export default function TransferSiswaBaruPage() {
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-slate-900">Transfer Data Siswa Baru</h1>
        <p className="text-sm text-slate-500 mt-1">
          Salin 7 kolom dari <code className="bg-slate-100 px-1 py-0.5 rounded text-xs">siswa_baru</code> ke
          ...
          <code className="bg-slate-100 px-1 py-0.5 rounded text-xs">siswa_baru</code> tidak dihapus.
        </p>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={13} className="px-4 py-10 text-center text-slate-400">
                  <UserPlus2 className="h-6 w-6 mx-auto mb-2 text-slate-300" />
                  {rows.length === 0
                  ...
                  <tr key={row.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-3 text-slate-600">
                      ...
                      {row.jarak_tempuh && row.jarak_tempuh !== jarakTempuhBersih && (
                        <span className="text-slate-400"> (asal: {row.jarak_tempuh})</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-slate-600">
                      {jarakRumahKode ?? "-"}
                      {row.jarak_rumah && <span className="text-slate-400"> ({row.jarak_rumah} km)</span>}
                    </td>
                    <td className="px-4 py-3">
                      {!nisn ? (
